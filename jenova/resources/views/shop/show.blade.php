    ...
    <div class="row">
        ...
        <div class="col col-12 col-md-6 mt-3 mt-md-0 px-5 pt-md-5 product-detail d-flex flex-column gap-2">
            ...
            <div class="row mt-1">
                ...
                <div class="col col-6">
                    <button type="submit" class="btn btn-danger btn-block rounded-3 p-2 px-3 fs-6 mt-2">
                        <i class="fa-solid fa-cart-shopping"></i> Add to Cart
                    </button>
                </div>
                </form>
            </div>
            <p class="mt-1"><i class="fas fa-shipping-fast"></i> $30 USD Shipping Cost Applied on all orders</p>
    ...
    <div class="row">
        <div class="col col-12">
            ...
            @forelse ($product->reviews as $review)
                <div class="review border p-3 mb-3 rounded-1 bg-danger-subtle">
                    <p><strong>{{ $review->email }}</strong> - Rating: {{ $review->rating }}/5</p>
                    <p>{{ $review->review }}</p>
                </div>
            @empty
                <p>No reviews yet.</p>
...
<div class="container py-5">
    ...
    @csrf
    <input type="hidden" name="customer_id" value="1"> <!-- Default customer_id value -->
    <input type="hidden" name="product_id" value="{{ $product->id }}"> <!-- Pass the product ID -->

    <div class="mb-3">
        <label for="email" class="form-label fw-semibold fs-5">Your Email</label>
    ...
    <button type="submit" class="btn btn-danger rounded-pill text-white">Submit Review</button>
</form>
        </div>
    </div>
</div>
    ...
    <div class="row py-5" id="products">
        ...
        <div class="col col-6 col-lg-3 col-xl-3">
            <div class="card h-100 border-0 text-center position-relative product-card overflow-hidden">
                ...
                <span class="badge bg-danger position-absolute top-0 start-0 m-3 rounded-5">{{ $similarProduct->discount_percentage }}%
                    OFF</span>
                @endif
                ...
                <div class="position-relative">
                    <img src="{{ asset('storage/' . $similarProduct->image_path) }}"
                        class="card-img-top rounded-5 transition" alt="{{ $similarProduct->name }}"  style="height: 250px;">
